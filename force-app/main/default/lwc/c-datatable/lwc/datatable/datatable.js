...
export default class Datatable extends LightningElement {
    ...
    handleGetInfo({ data, error }) {
        if (data) {
            ...
            if (isBlank(this.recordtypeDeveloperName)) {
                ...
                let run = async () => {
                    this.recordTypeId = await getRecordTypeIdFromDevName(this.recordtypeDeveloperName, this.objectApiName);
                }
                run();
            }
        } else if (error) {
        ...
        if (data) {
            let run = async () => {
                if (this.objectApiName && this.columns) {
                    ...
                    this.columns = await formatColumns(editableColumns, this.objectApiName);
                    ...
                    this.columns.forEach(column => {
                        ...
                        if (column.type === 'picklist' && column.editable !== undefined && column.editable &&
                            column.typeAttributes.options === undefined && [...picklistValuesMap.keys()].includes(fieldName)) {
                            let picklistOptions = [];
                            picklistValuesMap.get(fieldName).values.forEach(picklistValue => {
                                picklistOptions.push({ value: picklistValue.value, label: picklistValue.label });
                            });
    ...
    */
    ...
    @api show() {
        ...
        let run = async () => {
            const theQuery = await buildQuery(this.limitOfRecords, this.objectApiName, '', '', this.queryApexFields, this.searchByApiName, this.searchKey, this.whereClause, false, this.additionalSearchByFields);
            const queryCounter = buildQueryCounter(this.objectApiName, this.searchByApiName, this.searchKey, this.whereClause, this.additionalSearchByFields);
            getRecordsWithCounter({
                theQuery: theQuery,
                buildQueryCounter: queryCounter
            }).then(result => {
                this.totalNumberOfRecords = result.totalRecordCount;
                const newData = [...flattenRecords(copyRecordsIntoNewArray(result.records))]; // return an array with the data flattened
    ...
    loadMoreData(event) {
        ...
        this.delayTimeout = setTimeout(() => {
            isLoading = true;
            this.queryOffSet = this.queryOffSet + parseInt(this.limitOfRecords);
            if (this.records.length < this.totalNumberOfRecords) {//si hay records aun en la base de datos
                //Display "Loading" when more data is being loaded
                this.loadMoreStatus = 'Loading';
                let run = async () => {
                    const theQuery = await buildQuery(
                        this.limitOfRecords, this.objectApiName, this.queryOffSet.toString(), '', this.queryApexFields,
                        this.searchByApiName, this.searchKey, this.whereClause, false, this.additionalSearchByFields);
                    const queryCounter = buildQueryCounter(this.objectApiName, this.searchByApiName, this.searchKey,
                        this.whereClause, this.additionalSearchByFields);
                    getRecordsWithCounter({
                        theQuery: theQuery,
                        buildQueryCounter: queryCounter
                    }).then(result => {
                        infiniteLoading = false;
                        const currentData = [...flattenRecords(copyRecordsIntoNewArray(result.records))];
    ...
    handleSave() {
        ...
        var toReturn = this.draftValues.map(draftRecord => {
            ...
            inner.rowIdInTable = this.records.findIndex(record => draftRecord.Id === record.Id) + 1;
            return inner;
        });
    ...
    get optionsAreOK() {
        return this.options && this.options.length > 0 && this.limitOfRecords && this.displayModal;
    }
    @track displayModal;
    ...
    handleChangeLimitDisplayed(event) {
        this.limitOfRecords = event.detail.value;
        this.show();
        this.displayModal = false;
    }
    handleCancel(event) {
    ...
    handleCustomFieldEdit(event) {
        ...
        const column = this._columns.find(column => column.fieldName === fieldName);
        const fieldType = column.type;
        const popUpComponent = this.template.querySelector('[data-id="editCustomPopUpComponent"]');
    ...
    handleFieldChanged(event) {
        ...
        const newRecords = this.records.filter(record => record[fieldName] !== dataRecieved.value)
            .map(record => {
                return { ...record };
            });
        const recordsThatChanged = [];
        const selectedRecords = this.selectedRows.map(selectedRecord => selectedRecord.Id);

        for (let newRecord of newRecords) {
            const sameId = newRecord.Id === dataRecieved.recordId;
            if ((updateAlsoSelected && (sameId || selectedRecords.includes(newRecord.Id))) || sameId) {
        ...
        const column = this.columns.find(element => element.fieldName === fieldName);
        if (column.type === 'lookup') {
            ...
            if (newValue) {
                for (const record of newRecords) {
                    if ((selectedRecords.includes(record.Id) || record.Id === dataRecieved.recordId) && draftedRecords.includes(record.Id)) {
    ...
    pushDraftedValues(recordsThatChanged, fieldName, newValue) {
        ...
        let finalDrafted = [];
        for (const oldDraft of this.draftValues) {//pusheamos primero los viejos y los viejos que cambiaron
        ...
        for (const newDraftRecordId of recordsThatChanged) {//pusheamos los nuevos
            let draftToPush = {};
        ...
        for (const draft of finalDrafted) {
            ...
            for (const field of Object.getOwnPropertyNames(draft)) {
                if (databaseRecord[field] !== draft[field]) {//real drafted comparison is made against the databaseRecords
