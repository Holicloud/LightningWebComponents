...
export default class CustomDataTable extends LightningElement {
    ...
    */
    ...
    @api show() {
        ...
        let run = async () => {
            const theQuery = await buildQuery(this._limitOfRecords, this.objectApiName, '', '', this.queryApexFields, this.searchByApiName, this.searchKey, this.whereClause, false, this.additionalSearchByFields);
            ...
            if (this.records.length < this.totalNumberOfRecords) {//si hay records aun en la base de datos
                ...
                let run = async () => {
                    const theQuery = await buildQuery(
                        this._limitOfRecords, this.objectApiName, this.queryOffSet.toString(), '', this.queryApexFields,
    ...
    get optionsAreOK() {
        return this.options && this.options.length > 0 && !isBlank(this._limitOfRecords) && this.displayModal;
    }
    @track displayModal;
