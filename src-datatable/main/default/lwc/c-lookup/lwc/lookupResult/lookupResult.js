...
export default class LookupResult extends LightningElement {
  ...
  get primaryFieldDisplayableValue() {
    if (this.primaryField && this.record) {
      const value =
        this.record[
          Object.getOwnPropertyNames(this.record).find(
            (field) => field.toUpperCase() === this.primaryField
          )
        ];
      return this.searchKey
        ? this.boldOcurrences(this.searchKey, value)
        : value;
    }
  }
  ...
  boldOcurrences(searchKey, myValue) {
    ...
    while (indexOfItemFound !== -1) {
      const ocurrence = myValue.substring(
        indexOfItemFound,
        indexOfItemFound + searchKey.length
      );
      myValue = myValue.substring(
        myValue.indexOf(ocurrence) + searchKey.length
      );
