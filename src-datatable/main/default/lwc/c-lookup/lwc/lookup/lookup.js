...
export default class Lookup extends LightningElement {
  ...
  async showValue() {
    ...
    try {
      ...
      result.length
        ? this.throwEventSelected(result[0])
        : (this.recordSelected = null);
    } catch (error) {
  ...
  async handleOnchange(event) {
    ...
    if (isBlank(this.searchKey)) {
      await this[
        this.hideRecentRecords ? "showAllRecords" : "showRecentRecords"
      ]();
    ...
    this.delayTimeout = setTimeout(() => {
      getRecords({
        queryParameters: JSON.stringify({
          ...
          searchByFields: this._fields
            .filter((e) => e.searchable || e.primary)
            .map((e) => e.fieldName),
          whereClause: this.whereClause
        })
      })
   ...
   */
  handleSelect(event) {
    this.throwEventSelected(
      this.records.find((record) => event.detail === record.Id)
    );
  }
  ...
  throwEventSelected(recordSelected) {
    if (recordSelected) {
      this.inputValue =
        recordSelected[
          Object.getOwnPropertyNames(recordSelected).find(
            (property) => property.toUpperCase() === this.primaryField
          )
        ];
      this.recordSelected = recordSelected;
      this.dispatchEvent(
        new CustomEvent("selected", { detail: recordSelected })
      );
      this.submit();
    }
  }
